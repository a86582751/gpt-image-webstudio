...
- **自我迭代模式**：生成图片后调用多模态模型进行视觉评估，自动返回优化提示词进入下一轮；支持最多 10 组并行批量迭代。
...
`doubao-seedream-5-0-260128` 会被视为 Lite 模型的同名兼容 ID。程序会按官方 size string 表把当前“图片比例 + 分辨率”直接转换成固定宽高像素值传给接口，而不是传 `1K/2K/3K/4K` 档位让模型自行判断。例如 Pro 的“高清 + 16:9 宽屏”会传 `2848x1600`，Lite 的“高清 + 16:9 宽屏”会传 `4096x2304`。
