...
默认日志等级为 `INFO`，只显示模式、文本任务、图片保存和多模态结果等业务日志；底层请求成功记录不会重复显示。需要查看每次请求尝试和底层完成信息时，可以在 `.env` 中增加：
...
在设置页保存目录下方开启“保存每次文生图提示词”并保存设置后，应用会在当前图片输出目录创建或追加：
...
- 随机模式：每次随机提示词生成后追加一条记录。
- 创意模式：整个批次保存为一条记录，包含本批次生成的提示词 1、提示词 2、提示词 3……
- 自我迭代模式：整个批次保存为一条记录，每组只保存最后一轮的最终提示词，不保存初始提示词和中间轮次。
- 每条记录包含生成时间和创作方向，不保存任务编号或模型信息。

该开关默认关闭。关闭时不会创建提示词历史文件。
